...
namespace Morpheus {
	...
	Texture* CookTorranceLUTComputeKernel::submitUnmanaged(uint roughnessPixels, uint NoVPixels) {
		if (!mGPUBackend)
	...
	void CookTorranceLUTComputeKernel::init() {
		if (!mGPUBackend) {
			...
			mGPUBackend = loadEx<Shader>("/internal/cooktorrancelut.comp", params, this, true);
		}	
	}
}